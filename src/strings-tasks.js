...
function sumOfCodes(str) {
  return str ? str.split('').reduce((acc, el) => acc + el.charCodeAt(0), 0) : 0;
}
...
function countVowels(str) {
  ...
  return str.length < 0 || !str.match(regex) ? 0 : str.match(regex).length;
}
...
function isPalindrome(str) {
  return (
    ...
    str.replace(/\s|\W/g, '').toLowerCase().split('').reverse().join('')
  );
}
...
function getCardId(value) {
  ...
  return cardDeck.findIndex((card) => card === value);
}
